/**
 * Creates (or reuses) a local instructor user and prints a Bearer session token.
...
const DEV_EMAIL = 'dev-instructor@local.nibras';
const DEV_USERNAME = 'dev_instructor';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!instructorRole) {
      throw new Error(
        'Instructor role not seeded. Start the API once so roles seed runs.',
      );
    }
